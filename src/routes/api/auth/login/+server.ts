...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    if (userIndex === -1) {
      logActivity(event, 'unknown', email, 'login', 'failed', 'User not found');
    ...
    if (!bcrypt.compareSync(password, user.passwordHash)) {
      logActivity(event, user.id, user.email, 'login', 'failed', 'Incorrect password entered');
      return json({ error: 'Invalid email or password' }, { status: 401 });
    }
    ...
    if (user.twoFactorEnabled) {
      ...
      logActivity(event, user.id, user.email, 'two_factor_toggle', 'success', 'Two-Factor challenge initiated during login');
    ...
    logActivity(event, user.id, user.email, 'login', 'success', `Logged in successfully (rememberMe: ${!!rememberMe})`);
